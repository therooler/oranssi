...
from oranssi.optimizers import local_custom_su_lie_optimizer, parameter_shift_optimizer
...
def ps_optimization(eta, nsteps):
    ...
    costs_optimizer, states = parameter_shift_optimizer(circuit, params, observables, dev,
                                                            eta=eta, return_state=True, nsteps=nsteps)
    phi_optimizer = []
    theta_optimizer = []
    costs_converted = []
    print(states[0])
...
def riemann_optimization(eta, nsteps):
    ...
    costs_optimizer, states = local_custom_su_lie_optimizer(circuit, params, observables, dev, eta=eta,
                                                            layer_pattern=[(1, 0)], return_state=True, nsteps=nsteps)
    phi_optimizer = []
    theta_optimizer = []
    costs_converted = []
    print(states[0])
