...
def two_observables_2_qubits():
    ...
    axs[2].plot(costs_exact_p, label=r'Lie $SU(2^n)$ Pert.', color=cmap(0.3), zorder=-1)
    axs[2].plot(costs_exact_unp, label=r'Lie $SU(2^n)$', color=cmap(0.2), zorder=-1)
    axs[2].plot(range(len(costs_exact_p)), [eigenvalues[0] for _ in range(len(costs_exact_p))],
             label='Min.',
             color='gray', linestyle='--', zorder=-1)
    for i, p in enumerate(perturbations):
        if i == 0:
            axs[2].scatter(p, costs_exact_p[p], color=cmap(0.1), label='Perturbation', s=15, zorder=1)
        else:
            axs[2].scatter(p, costs_exact_p[p], color=cmap(0.1), s=15, zorder=1)
